...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://postgres:Soulfood123@localhost:5432/HW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
def get_gifs_from_giphy(search_string):
    ...
    urlparams={}
    urlparams['q']=search_string
    urlparams['api_key']=api_key
    urlparams['limit']=5
    results=requests.get(baseurl,params=urlparams)
...
def get_or_create_search_term(db_session, term):
    ...
    if searchTerm:
        ...
        for g in get_gifs_from_giphy(term):
            gif_iteration=get_or_create_gif(db_session, g[0],g[1])

            searchTerm.gifs.append(gif_iteration)
...
def create_collection():
    ...
    if form.validate_on_submit():
        ...
        gif_object = [get_gif_by_id(db.session, int(id)) for id in listIDs]
        get_or_create_collection(db.session, form.name.data, current_user, gif_object)
